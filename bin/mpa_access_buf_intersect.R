...
d1_n<-d1%>%
  ...
  st_filter(d8)%>%
  st_join(d8)%>%
  select(id_pap,id_nms,nms_name=SANCTUARY,nms_full_name=FullName)%>%
  mutate(nms_250m=1)%>%
  glimpse()
# plot(d1_n)
...
d2_n<-d2%>%
  ...
  st_filter(d8)%>%
  st_join(d8)%>%
  select(id_ppk,id_nms,nms_name=SANCTUARY,nms_full_name=FullName)%>%
  mutate(nms_250m=1)%>%
  glimpse()
# plot(d2_n)
...
d3_n<-d3%>%
  ...
  st_filter(d8)%>%
  st_join(d8)%>%
  select(id_paj,id_nms,nms_name=SANCTUARY,nms_full_name=FullName)%>%
  mutate(nms_250m=1)%>%
  glimpse()
# plot(d3_n)
